			...
			<div id="table-wrapper">
				<table id="reserveViewTable">
					...
					<tr>
						...
						<td>${rdto.r_total}</td>
						<th>지불상태</th>
						<td><c:if test="${rdto.yes==0}">입금대기</c:if> <c:if
								test="${rdto.yes==1}">입금완료</c:if></td>
					</tr>
				...
				<table id="personViewTable">
					...
					<tr id="contentTr">
						<th>고객 요청사항</th>
						<td colspan=3 style="text-align: left; vertical-align: top;"><c:if
								test="${pdto.p_contents==null}">요청사항 없음</c:if> <c:if
								test="${pdto.p_contents!=null}"> ${pdto.p_contents} </c:if></td>
					</tr>
				</table>
			</div>
		</div>
